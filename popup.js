...
function authenticate(){
    chrome.runtime.sendMessage({type: 3}, function() {
    });
}

function disconnectTwitch(){
    chrome.runtime.sendMessage({type: 4}, function() {
        onForceUpdate();
    });
}
...
function updateTable() {
    chrome.storage.local.get({sortMethod: {}, streamers:{}, access_token: ''}, function (result) {
...
function changeSort(newSelection) {
	chrome.storage.local.get({sortMethod: {}}, function(result) {
		...
		chrome.storage.local.set({sortMethod: newSortMethod}, function () {
			updateTable();
		});
        });
}
